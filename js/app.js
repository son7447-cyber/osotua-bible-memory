...
async function loadSettings(){
  const{data,error}=await sb.from("project_settings")
    .select("current_day,auto_advance,start_date")
    .eq("id",1).single();
  if(error)throw error;
  settings=data;
  currentDay=data.auto_advance?calculatedDay(data.start_date):data.current_day;
  $("dayModeBadge").textContent=data.auto_advance?"Automatic":"Manual";
  $("dayModeBadge").className=`mode-badge ${data.auto_advance?"auto":""}`;
}
...
async function loadParticipants(){
  ...
  if(error)throw error;
  participants=data;
  participantMap=new Map(data.map(x=>[x.id,x.name]));
...
async function submitRecording(){
  ...
  const path=`${name.replace(/[^a-zA-Z0-9_-]/g,"_")}/day-${currentDay}-${Date.now()}.webm`;
  try{
    $("submitStatus").textContent="Uploading…";
    const{error:u}=await sb.storage.from(OSOTUA_CONFIG.bucket).upload(path,audioBlob,{contentType:audioBlob.type||"audio/webm"});
    if(u)throw u;

    const{data:existing,error:q}=await sb.from("memory_submissions")
      .select("id").eq("participant_id",pid).eq("day",currentDay).maybeSingle();
    if(q)throw q;
    ...
    const r=existing
      ...
      : await sb.from("memory_submissions").insert({...payload,participant_id:pid,day:currentDay});
...
$("nextDay").onclick=()=>{
  currentDay=Math.min(50,currentDay+1);
  $("adminCurrentDay").textContent=`Day ${currentDay}`;
};
